...
def handle_emergency(api_key: str, prompt: str, history: list, file_data: dict = None, profile: dict = None) -> str:
    ...
    system_instruction = f"""You are an evidence-based maternal healthcare AI assistant specializing in urgent warning signs (heavy bleeding, chest pain, pre-eclampsia signs, lack of baby movement).
    
    Use the retrieved medical context below as your primary source of truth.
    Do not invent medical facts.
    If the retrieved context is insufficient, clearly instruct the user to contact emergency services immediately.
    Focus 100% on safety, caution, and direct action.
    Always prioritize the retrieved context and recommend immediate medical attention.
    ...
    Answer the user's question safely, adhering to the retrieved guidelines. Recommend immediate contact with emergency healthcare services or their OB/GYN."""
    ...
    try:
        ...
        return f"Error executing emergency agent: {e}"
