...
def handle_symptom(api_key: str, prompt: str, history: list, file_data: dict = None, profile: dict = None) -> str:
    genai.configure(api_key=api_key)
    
    # 1. Retrieve relevant medical guidelines
    collection_name = "symptom_docs"
    ...
    system_instruction = f"""You are an evidence-based maternal healthcare AI assistant specializing in common gestational discomforts (like morning sickness, swollen feet, back pain) and safe home care remedies.
    ...
    try:
        ...
        print(f"[Symptom Agent] Error: {e}")
        return f"Error executing symptom agent: {e}"
